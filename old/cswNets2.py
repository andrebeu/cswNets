...
from protoCSW import gen_data_dict
from protoCSW import gen_data_dict2
...
class BaseRNN():
  ...
  def setup_placeholders(self,xph_dim,yph_dim):
    ...
    self.dropout_keep_prob = tf.placeholder(tf.int64,
                  shape=[],
                  name="dropout_keep_prob_placeholder")
  ...
  def get_optimizer(self):
    ...
    """
    with self.graph.as_default():
      optimizer_op = tf.train.GradientDescentOptimizer(0.05)
  ...
  def train(self,RAND,COND,train_info):
    ...
    """ num_paths controls how many paths in one story
      num_stories controls how many stories per dataset
      dimension is [num_stories, num_paths*path_len(3)] """
    num_paths = int(self.num_sequences/3)
    csw_data_dict = gen_data_dict2(num_stories=100,num_paths=num_paths,graph_dict=None)
    for epoch_num in range(num_epochs):
      epoch_t1 = dt.now()
      # proto CSW data generate
      # csw_data_dict = gen_data_dict(100,RAND,COND)
      train_feed_dict = {self.xph:csw_data_dict['X_data'],
                         self.yph:csw_data_dict['Y_data'],
      ...
      self.sess.run([self.itr_initop],train_feed_dict)
      # train loop
      ...
      if epoch_num%(num_epochs//NUM_OF_EVALS)==0: 
        eval_acc = self.eval(RAND,COND,csw_data_dict=csw_data_dict)
  ...
  def eval(self,RAND,COND,csw_data_dict,sess=None):
    """ makes predictions and computes loss
    """
    if not sess: sess = self.sess
    # feed dictionary
    # eval_data_dict = gen_data_dict(100,RAND,CON
    # eval_data_dict = gen_data_dict(100,RAND,CON
    eval_feed_dict = {self.xph:csw_data_dict['X_data'], 
                      self.yph:csw_data_dict['Y_data'], 
...
class RNNseq2(BaseRNN):
  ...
  def setup_inference(self,x_batch):
    ...
    # setup RNN cell
    self.cell = cell = tf.contrib.rnn.BasicRNNCell(self.celldim)
    self.cell = cell = tf.contrib.rnn.DropoutWrapper(cell,
                        input_keep_prob=self.dropout_keep_prob,
                        output_keep_prob=1.00,
                        state_keep_prob=1.00)
    # initialize state
...
class LSTMseq2(BaseRNN):
  ...
  def setup_inference(self,x_batch):
    ...
    """
    # setup cell
    cell = tf.contrib.rnn.LayerNormBasicLSTMCell(self.celldim)
    self.cell = cell = tf.contrib.rnn.DropoutWrapper(cell,
                        input_keep_prob=self.dropout_keep_prob,
                        output_keep_prob=1.00,
                        state_keep_prob=1.0)
    # self.cell = cell = ZoneoutWrapper(cell,state_zoneout_prob=(0.05, 0))
